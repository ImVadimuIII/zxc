...
def setup_handlers(bot):
    ...
    def delete_last_message(chat_id):
        ...
        if chat_id in user_last_message:
            try:
                ...
                del user_last_message[chat_id]  # Usuwamy zapis ID po usunięciu wiadomości
    ...
    def send_and_track_message(chat_id, text, reply_markup=None, parse_mode=None):
        ...
        # Usuwamy poprzednią wiadomość, jeśli istnieje
        delete_last_message(chat_id)

        # Wysyłamy nową wiadomość
        try:
            sent_message = bot.send_message(chat_id, text, reply_markup=reply_markup, parse_mode=parse_mode)
            user_last_message[chat_id] = sent_message.message_id  # Zapisujemy ID nowej wiadomości
        except Exception as e:
    ...
    def handle_unexpected_messages(message):
        """Obsługuje wszystkie inne сообщения."""
        bot.send_message(
            message.chat.id,
            "Ошибка! Для продолжения используйте команду /start."
        )
    ...
    def show_main_menu(message):
        ...
        row2 = [
            types.InlineKeyboardButton("🎾 Целевая тренировка", callback_data="Целевая тренировка"),
    ...
    @bot.callback_query_handler(func=lambda call: call.data == "Паурлифтинг и силовые")
    def handle_powerlifting_and_strength(call):
        bot.answer_callback_query(call.id)
        markup = types.InlineKeyboardMarkup()
        row1 = [
    ...
    # Obsługuje akcje w sekcji "Паурлифтинг и силовые"
    ...
    @bot.callback_query_handler(func=lambda call: call.data == "Авторы")
    def handle_authors(call):
        bot.answer_callback_query(call.id)
        markup = types.InlineKeyboardMarkup()
        row1 = [
 ...
 \
    ...
    # Random training
    @bot.callback_query_handler(func=lambda call: call.data == "Целевая тренировка")
